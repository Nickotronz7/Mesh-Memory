...
/**
 * Codifica el string que se le introduzca
 * @param Str
 * @param in_len
...
string decode(string const& code)
{
    ...
    while (in_len-- && ( code[in_] != '=') && is_base64(code[in_])) {
        char_array_4[i++] = code[in_]; in_++;
        if (i ==4) {
            for (i = 0; i <4; i++)
